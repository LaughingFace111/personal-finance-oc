...
import { Button, Card, Form, Input, InputNumber, Radio, Spin, message } from 'antd'
  ...
  return (
    <Card style={{ borderRadius: 16, borderColor: 'var(--border-color)', background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 6, color: 'var(--text-secondary)' }}>Phase 1 只支持总预算，自然月和自定义时间范围。</div>
      </div>

      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        <Form.Item name="name" label="预算名称" rules={[{ required: true, message: '请输入预算名称' }]}>
          <Input placeholder="例如：4 月总预算" maxLength={100} />
        </Form.Item>
